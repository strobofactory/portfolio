...
function buildLanguageSelector() {
  ...
  select.innerHTML = options.map(([code,href,label]) => `<option value="${href}"${code===locale?' selected':''}>${label}</option>`).join('');
  ...
  if (!document.getElementById('language-select-style')) {
    ...
    style.textContent = `.language-select{appearance:none;-webkit-appearance:none;border:1px solid var(--line);background:#fff;color:var(--ink);font:inherit;font-size:10px;letter-spacing:.04em;padding:8px 28px 8px 10px;cursor:pointer;border-radius:0;background-image:linear-gradient(45deg,transparent 50%,var(--ink) 50%),linear-gradient(135deg,var(--ink) 50%,transparent 50%);background-position:calc(100% - 12px) 50%,calc(100% - 8px) 50%;background-size:4px 4px,4px 4px;background-repeat:no-repeat}.language-select:focus{outline:1px solid var(--accent);outline-offset:2px}@media(max-width:760px){.language-select{font-size:9px;padding:7px 26px 7px 9px;max-width:112px}}`;
